...
namespace System.Process.UnitTests.Application.Commands.StatementTypes
{
    public class StatementTypesCommandTests
    {
        ...
        [Fact(DisplayName = "Should Send Handle StatementTypes Error")]
        public async Task ShouldSendHandleStaementsTypesError()
        {
            var statementReadRepository = new Mock<IStatementReadRepository>();
            var recordTypeClient = new Mock<IGetRecordTypeClient>();
            var getTokenParams = new Mock<IOptions<GetTokenParams>>();
            ...
            statementReadRepository
                .Setup(r => r.FindBy(It.IsAny<bool>(), It.IsAny<bool>(), It.IsAny<bool>())).Throws(new Exception());
            ...
            await Assert.ThrowsAsync<UnprocessableEntityException>(() => statementsTypesCommand.Handle(request, cancellationToken));
        }
